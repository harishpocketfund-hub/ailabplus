...
import {
  createDevelopmentProjectId,
  DEFAULT_DEVELOPMENT_TAGS,
  getDevelopmentProjectsServerSnapshot,
  getDevelopmentProjectsSnapshot,
  getDevelopmentTagsServerSnapshot,
  getDevelopmentTagsSnapshot,
  DevelopmentProject,
  DEVELOPMENT_PROJECT_PRIORITY_OPTIONS,
  DevelopmentProjectPriority,
  parseDevelopmentProjects,
  parseDevelopmentTags,
  subscribeToDevelopmentProjects,
  subscribeToDevelopmentTags,
  writeDevelopmentProjects,
  writeDevelopmentTags,
...
type ProjectTypeFilter =
  ...
  | "Overdue project"
  | "On track project";
type ProjectPriorityFilter = "All" | DevelopmentProjectPriority;

type DevelopmentProjectFilters = {
  search: string;
...
const createDefaultFilters = (): DevelopmentProjectFilters => ({
  search: "",
  deadline: "Any",
  person: "All",
  type: "All",
...
function parseFilters(rawFilters: string | null): DevelopmentProjectFilters {
  if (!rawFilters) {
    return createDefaultFilters();
  }

  try {
    const parsed = JSON.parse(rawFilters) as Partial<DevelopmentProjectFilters> & {
      status?: string;
      risk?: string;
    };
...
function getProjectPriorityBadgeClasses(priority: DevelopmentProjectPriority): string {
  if (priority === "High") {
...
export default function DevelopmentPage() {
  ...
  const storageTags = parseDevelopmentTags(rawTags);
  ...
  const [projectPriority, setProjectPriority] =
    useState<DevelopmentProjectPriority>("Medium");
  ...
  const onEditOpen = (project: DevelopmentProject) => {
    setIsCreating(true);
  ...
  const onSubmitProject = (event: FormEvent<HTMLFormElement>) => {
    ...
    writeDevelopmentProjects(updatedProjects);
    ...
    if (existingProject) {
      ...
      appendDevelopmentProjectCommitLogs(nextCommitLogs);
    }
    ...
    <section className="w-full max-w-7xl space-y-4">
      <div className="rounded-2xl border border-black/10 bg-gradient-to-b from-white to-black/[0.02] p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight">Development Portfolio</h1>
              ...
              <div>
                <p className="inline-flex items-center gap-1 text-xs font-medium text-black/60">
                  ...
                  {editingProjectId
                    ...
                    : "Create a new Development initiative"}
                </p>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <label className="block text-sm">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setProjectPriority(event.target.value as DevelopmentProjectPriority)
                      }
                      className="mt-1.5 block h-10 w-full rounded-md border border-black/20 px-3"
                    >
                      {DEVELOPMENT_PROJECT_PRIORITY_OPTIONS.map((priority) => (
