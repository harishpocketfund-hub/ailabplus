...
import {
  ...
  writeDevelopmentMembersForProject,
...
import {
  createDevelopmentSubtaskId,
  createDevelopmentTaskId,
  ...
  normalizeDevelopmentTaskOrders,
...
type TaskPriorityFilterValue = DevelopmentTaskPriority | typeof ALL_FILTER_VALUE;
type TaskStatusFilterValue = DevelopmentTaskStatus | typeof ALL_FILTER_VALUE;
...
type WeekDateEntry = {
  weekday: DevelopmentRecurringWeekday;
  date: string;
};
...
const getColumnId = (status: DevelopmentTaskStatus): string => `column:${status}`;
...
): DevelopmentTaskStatus | null => {
  if (typeof id !== "string" || !id.startsWith("column:")) {
    return null;
  }

  const status = id.slice("column:".length) as DevelopmentTaskStatus;
...
const getPriorityBadgeClasses = (priority: DevelopmentTaskPriority): string => {
  if (priority === "High") {
...
const getStatusBadgeClasses = (status: DevelopmentTaskStatus): string => {
  if (status === "Done") {
    return "border-green-200 bg-green-100 text-green-700";
  }
  if (status === "In Progress") {
...
const isTaskPriorityFilterValue = (
  ...
  return (
    ...
    TASK_PRIORITY_OPTIONS.includes(value as DevelopmentTaskPriority)
  );
};
...
const isTaskStatusFilterValue = (value: unknown): value is TaskStatusFilterValue => {
  return (
    value === ALL_FILTER_VALUE ||
    TASK_STATUS_OPTIONS.includes(value as DevelopmentTaskStatus)
  );
};
...
const getWeekdayFromDate = (date: Date): DevelopmentRecurringWeekday => {
  const weekdayByIndex: DevelopmentRecurringWeekday[] = [
    "Sun",
    "Mon",
    "Tue",
    "Wed",
...
type TaskCardProps = {
  ...
  onMoveTask: (taskId: string, status: DevelopmentTaskStatus) => void;
};
...
export default function DevelopmentProjectPage() {
  ...
  const allProjectCommitLogs = parseDevelopmentProjectCommitLogs(rawProjectCommitLogs);
  const project = projects.find((item) => item.id === projectId);
  const tasksByProject = parseDevelopmentTasksByProject(rawTasksByProject);
  const membersByProject = parseDevelopmentMembersByProject(rawMembersByProject);
  const tasks = useMemo(
  ...
  const [taskPriority, setTaskPriority] = useState<DevelopmentTaskPriority>("Medium");
  const [isRecurringTask, setIsRecurringTask] = useState(false);
  const [recurringDays, setRecurringDays] = useState<DevelopmentRecurringWeekday[]>([]);
  ...
  const [createSubtasks, setCreateSubtasks] = useState<DevelopmentSubtask[]>([]);
  ...
  const [modalStatus, setModalStatus] = useState<DevelopmentTaskStatus>("To Do");
  ...
  const [modalPriority, setModalPriority] = useState<DevelopmentTaskPriority>("Medium");
  ...
  const [modalRecurringCompletions, setModalRecurringCompletions] =
    useState<DevelopmentRecurringCompletions>({});
  const [modalSubtasks, setModalSubtasks] = useState<DevelopmentSubtask[]>([]);
  ...
  const tasksByStatus = useMemo(() => {
    return TASK_STATUS_OPTIONS.reduce<Record<DevelopmentTaskStatus, DevelopmentTask[]>>(
      (accumulator, status) => {
  ...
  const filteredTasksByStatus = TASK_STATUS_OPTIONS.reduce<
    Record<DevelopmentTaskStatus, DevelopmentTask[]>
  >(
    (accumulator, status) => {
  ...
  const timelineTasksByStatus = useMemo(() => {
    const statusesForTimeline =
      statusFilter === ALL_FILTER_VALUE ? TASK_STATUS_OPTIONS : [statusFilter];
    const timelineByStatus: Record<DevelopmentTaskStatus, TimelineTaskEntry[]> = {
      "To Do": [],
      "In Progress": [],
      Review: [],
      Done: [],
    ...
    statusesForTimeline.forEach((status) => {
      const tasksWithDueDate = filteredTasksByStatus[status]
        ...
        .filter(
          (entry): entry is { task: DevelopmentTask; dueDateMs: number } =>
            entry !== null
        )
  ...
  const tasksNeedingAttention = useMemo(() => {
    const selected: DevelopmentTask[] = [];
  ...
  const writeProjectTasks = (nextTasks: DevelopmentTask[]) => {
    writeDevelopmentTasksForProject(projectId, normalizeDevelopmentTaskOrders(nextTasks));
  };

  const writeProjectMembers = (nextMembers: DevelopmentMember[]) => {
    writeDevelopmentMembersForProject(projectId, nextMembers);
  };
  ...
  const toggleCreateRecurringDay = (day: DevelopmentRecurringWeekday) => {
  ...
  const toggleModalRecurringDay = (day: DevelopmentRecurringWeekday) => {
  ...
  const openTaskModal = (task: DevelopmentTask) => {
  ...
  const openTaskFromSummary = (task: DevelopmentTask) => {
  ...
  const onMoveTask = (taskId: string, status: DevelopmentTaskStatus) => {
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    const movedTask: DevelopmentTask = {
      ...draggedTask,
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <label className="text-sm">
                      ...
                      <select
                        value={taskPriority}
                        onChange={(event) =>
                          setTaskPriority(event.target.value as DevelopmentTaskPriority)
                        }
                        className="mt-1 w-full rounded-md border border-black/20 px-3 py-2"
                      >
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="text-sm">
                    ...
                    <select
                      value={modalPriority}
                      onChange={(event) =>
                        setModalPriority(event.target.value as DevelopmentTaskPriority)
                      }
                      className="mt-1 w-full rounded-md border border-black/20 px-3 py-2"
                    >
                      {TASK_PRIORITY_OPTIONS.map((option) => (
                  ...
                  <label className="text-sm">
                    ...
                    <select
                      value={modalStatus}
                      onChange={(event) =>
                        setModalStatus(event.target.value as DevelopmentTaskStatus)
                      }
                      className="mt-1 w-full rounded-md border border-black/20 px-3 py-2"
                    >
